...
def generate_drone_commands(
    ...
    target_bbox_area_percentage: float = 0.15, # Целевая площадь bounding box объекта как процент от площади кадра
    tolerance_pixels: int = 20 # Допустимое отклонение центра объекта от центра кадра до генерации команды движения
) -> Dict[str, str]:
    """
    ...
    Args:
        ...
        target_bbox_area_percentage (float): Желаемая площадь ограничивающей рамки объекта,
                                             выраженная как доля (от 0 до 1) от общей площади
                                             исходного кадра. По умолчанию 0.15 (15%).
        tolerance_pixels (int): Максимальное смещение центра объекта от центра кадра
                                по любой из осей (горизонтальной/вертикальной),
                                при котором команда движения не генерируется (считается "NONE").
    ...
    # 1. Горизонтальное управление: корректировка позиции дрона по оси X
    if obj_cx < original_frame_center_x - tolerance_pixels:
        commands["horizontal"] = "LEFT" # Объект левее центра, дрон должен двигаться влево
    elif obj_cx > original_frame_center_x + tolerance_pixels:
        commands["horizontal"] = "RIGHT" # Объект правее центра, дрон должен двигаться вправо

    # 2. Вертикальное управление: корректировка позиции дрона по оси Y
    if obj_cy < original_frame_center_y - tolerance_pixels:
        commands["vertical"] = "UP" # Объект выше центра, дрон должен двигаться вверх
    elif obj_cy > original_frame_center_y + tolerance_pixels:
        commands["vertical"] = "DOWN" # Объект ниже центра, дрон должен двигаться вниз

    # 3. Управление дистанцией: приближение или отдаление от объекта
    ...
    # Используются диапазоны 85%-115% от целевой площади для небольшой "мертвой зоны"
    if current_obj_area < target_obj_area * 0.85: # Если текущая площадь объекта значительно меньше целевой
        commands["distance"] = "FORWARD"
    elif current_obj_area > target_obj_area * 1.15: # Если текущая площадь объекта значительно больше целевой
...
def track_video_and_center_object(
    ...
    target_imgsz: int = 640,  # Размер стороны квадратного кадра, который будет вырезан и сохранен
    confidence_threshold: float = 0.25, # Порог уверенности для детекции
    iou_threshold: float = 0.7 # Порог Intersection over Union (IoU) для не-максимального подавления (NMS)
) -> None:
    """
    ...
    os.makedirs(output_dir, exist_ok=True) # Создаем папку, если ее нет. exist_ok=True предотвращает ошибку, если папка уже есть.
    ...
    final_video_output_path = video_output_path # Путь, который будет реально использован

    for codec_fourcc_str, ext in codec_options:
        temp_video_output_path = video_output_path.rsplit('.', 1)[0] + ext # Обновляем расширение файла
        try:
            print(f"Попытка инициализации VideoWriter с кодеком '{codec_fourcc_str}' и расширением '{ext}'...")
            fourcc = cv2.VideoWriter_fourcc(*codec_fourcc_str) # type: ignore # Pylance может ругаться, но это стандартный синтаксис OpenCV
            out = cv2.VideoWriter(temp_video_output_path, fourcc, fps, (target_imgsz, target_imgsz))
            if out.isOpened():
                final_video_output_path = temp_video_output_path # Если успешно, сохраняем этот путь
        ...
        out = None # Сбрасываем out, если текущая попытка не удалась

    # Если ни один из кодеков не сработал, выходим
    if out is None or not out.isOpened():
        ...
        cap.release() # Освобождаем ресурсы VideoCapture
        return # Выход из функции

    # Обновляем video_output_path на тот, который фактически используется (с корректным расширением)
    ...
    log_file = None # Инициализируем None для безопасного закрытия
    try:
        ...
        # Освобождаем уже открытые ресурсы перед выходом
        cap.release()
        if out: out.release()
        return
    ...
    try:
        while True:
            ...
            if not ret:
                ...
                break # Выходим из цикла, если кадры закончились или ошибка

            frame_count += 1
            # Выводим прогресс каждые 100 кадров
            if total_frames > 0: # Избегаем деления на ноль, если total_frames неизвестен
            ...
            # Используем ByteTrack для стабильности, как обсуждалось ранее.
            ...
            if results and len(results) > 0 and results[0].boxes is not None and results[0].boxes.id is not None and len(results[0].boxes.id) > 0:
                ...
                if target_track_id is not None:
                    ...
                    for i in range(len(boxes)):
                        ...
                        track_id = int(boxes.id[i].cpu().item()) # type: ignore # Pylance может ругаться, но это корректно
                        if track_id == target_track_id: # Если найден наш целевой объект
                            ...
                            current_frame_data["status"] = tracking_status # Обновляем статус в данных
                            found_target_in_current_frame = True
                            break # Нашли целевой объект, больше не ищем в этом кадре
                    
                    if not found_target_in_current_frame:
                        # Если целевой объект не найден в текущем кадре (возможно, временная потеря)
                        ...
                        current_frame_data["status"] = "Потерян" # Обновляем статус

                # Если целевой ID не задан (это первый кадр, или объект был потерян
                # и нужно выбрать новый), или если мы искали, но не нашли старый ID:
                if current_target_bbox is None:
                    ...
                    if selected_new_target and target_track_id is None:
                        # Первый кадр, выбран целевой сноубордист
                        ...
                        # Целевой объект с прежним ID потерян, и выбран новый (крупнейший) объект
                        ...
                        target_track_id = current_target_bbox[4] # type: ignore # Обновляем целевой ID
                        current_frame_data["confidence"] = current_target_bbox[5] # type: ignore
                        current_frame_data["tracked_id"] = current_target_bbox[4] # type: ignore
                        current_frame_data["status"] = tracking_status
                    elif current_target_bbox is None:
                        # В текущем кадре вообще не найдено объектов класса 'snowboarder'
            ...
            if current_target_center is not None and current_target_bbox is not None:
                ...
                target_obj_area = original_total_frame_area * 0.15 # Целевые 15% от площади исходного кадра
                ...
                # Статус трекинга уже обновлен выше, просто убеждаемся, что он есть в данных
                current_frame_data["status"] = tracking_status
            else:
                # Если целевой объект не найден в текущем кадре, используем "Поиск" статус
                current_frame_data["status"] = tracking_status
            ...
            # Если целевой сноубордист еще не был найден ни разу, записываем черный кадр
            ...
            cx, cy = last_known_center # Центр объекта в исходном кадре
            ...
            if actual_crop_width > 0 and actual_crop_height > 0:
                ...
                # (иногда возникают проблемы с индексацией NumPy/OpenCV)
        ...
        # Продолжаем выполнять finally блок
        ...
        # Финальные сообщения о завершении
